...
df_filt <- read.csv(file = 'three_metrics_for_cart_plus.csv')
colnames(df_filt)
...
#use below for Net Benefits
df_filt$costs_bin <- ifelse(df_filt$Net.Benefits.Calculated.from.Cooperation.plus <= 0, "Low", "High")
...
fit_class <- rpart(costs_bin ~ .,
                   method="class", data=df_class,control=c(maxdepth = 7, minsplit = 2), cp=.000001)
